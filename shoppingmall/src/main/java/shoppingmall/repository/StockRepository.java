   ...
   //주식 표 출력
	...
	public List<StockA3> recommandStock(String memberNum){
		...
		sql = " SELECT S1.* , CASE WHEN SYMBOL = 'KR7005930003' THEN '삼성전자' "
				+ "                   ELSE '-' END STOCK_NAME    "
				+ "FROM ( SELECT s.*,ROW_NUMBER() OVER ( PARTITION BY TO_CHAR(trading_date, 'yyyy-MM-dd'), trading_hours   "
				+ "                                      ORDER BY TO_CHAR(trading_date, 'yyyy-MM-dd') DESC, trading_hours DESC, ROWID DESC "
				+ "                                      ) AS rn    "
				+ "        FROM stock1 s    "
				+ "        WHERE TRUNC(trading_date) = TRUNC(SYSDATE) )S1  "
				+ "        WHERE rn = 1  and SYMBOL = (  SELECT CASE WHEN STOCK_NAME = '삼성전자' THEN 'KR7005930003'              "
				+ "                                                  ELSE '-' END STOCK_NAME   "
				+ "                                      FROM GOODS  "
				+ "                                      WHERE STOCK_NAME = ( select STOCK_NAME from (select STOCK_NAME  , COUNT(*) OVER (PARTITION BY STOCK_NAME) AS cnt  "
